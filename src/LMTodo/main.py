from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QHBoxLayout, QFrame, QVBoxLayout, QPushButton, QListWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        if not any(screen.geometry().intersects(saved_geometry) for screen in QGuiApplication.screens()):
    ...
    def set_projects_buttons_state(self):
        if not self.project_list.selectedIndexes():
            ...
            self.all_projects_btn.setStyleSheet("background-color: palette(highlight); color: palette(highlightedText); font-weight: bold;")
    ...
    def add_project(self):
        ...
        btn_pos = self.add_project_btn.mapToGlobal(self.add_project_btn.rect().bottomLeft())
        bubble.move(btn_pos.x() - 10, btn_pos.y() - bubble.height() - 30)

        def on_add():
            name = bubble.name_input.text().strip()
            if name:
                ThreadRunner(todo_controller.add_project, self.load_projects, name).start()
                bubble.close()
    ...
    def edit_project(self):
        ...
        bubble = BubbleWidget(self, translate("Edit project name:"), translate("Save"), self.edit_project_btn, initial_text=current_name)
        btn_pos = self.edit_project_btn.mapToGlobal(self.edit_project_btn.rect().bottomLeft())
        bubble.move(btn_pos.x() - 10, btn_pos.y() - bubble.height() - 30)
        ...
        def on_save():
            ...
            if not name:
                return
            ThreadRunner(todo_controller.edit_project, self.load_projects, project_id, name).start()
            bubble.close()
    ...
    def delete_project(self):
        ...
        bubble = BubbleWidget(self, f"{translate('Delete project')} '{project_name}'?", translate("Delete"), self.delete_project_btn, show_input=False, cancel_text=translate("Cancel"), warning_text=translate("This action can't be undone."))
        btn_pos = self.delete_project_btn.mapToGlobal(self.delete_project_btn.rect().bottomLeft())
        bubble.move(btn_pos.x() - 10, btn_pos.y() - bubble.height() - 30)

        def on_confirm():
            ThreadRunner(todo_controller.delete_project, self.load_projects, project_id).start()
            bubble.close()
    ...
    def toggle_config_panel(self):
        if self.config_panel.isVisible():
            ...
            self.config_btn.setStyleSheet("background-color: palette(highlight); color: palette(highlightedText); font-weight: bold;")
    ...
    def set_shortcuts(self):
        ...
        self.task_panel.filter_widget.buttons['All'].setShortcut(shortcuts["filter_all"]) # ["All", "On Time", "Overdue", "Open", "Finished", "Cancelled"]
        self.task_panel.filter_widget.buttons['On Time'].setShortcut(shortcuts["on_time"])
        self.task_panel.filter_widget.buttons['Overdue'].setShortcut(shortcuts["overdue"])
        self.task_panel.filter_widget.buttons['Open'].setShortcut(shortcuts["filter_active"])
        self.task_panel.filter_widget.buttons['Finished'].setShortcut(shortcuts["filter_completed"])
        self.task_panel.filter_widget.buttons['Cancelled'].setShortcut(shortcuts["filter_canceled"])
        self.select_projects_shortcut = QShortcut(QKeySequence(shortcuts["select_project"]), self)
        self.select_projects_shortcut.activated.connect(self.project_list.setFocus)
        self.select_tasks_shortcut = QShortcut(QKeySequence(shortcuts["select_tasks"]), self)
